...
export function RFIsGanttChart({ rfis }) {
  ...
  const years = useMemo(() => {
    ...
    return Array.from(seen).map((y) => {
      const yNum = +y;
      const ys =
        new Date(yNum, 0, 1) < startDate ? startDate : new Date(yNum, 0, 1);
      const ye =
        new Date(yNum, 11, 31) > endDate ? endDate : new Date(yNum, 11, 31);
      return {
        year: y,
            ...
            <div className="relative">
              <div
                ...
                style={{
                  ...
                  height: `${Object.values(rfisByUser).reduce(
                    (s, a) => s + a.length * 36 + 8,
                    0
                  )}px`,
                }}
