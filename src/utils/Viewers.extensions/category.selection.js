...
class CategorySelectionExtension extends Autodesk.Viewing.Extension {
    ...
    onToolbarCreated() {
      ...
      this._group = this.viewer.toolbar.getControl('TADCustomControls') ||
      ...
      this._button.onClick = () => {
        ...
        this.viewer.getProperties(baseDbId, (result) => {
          ...
          const implVisibilityMgr =
            ...
            (this.viewer.model && this.viewer.model.getVisibilityManager && this.viewer.model.getVisibilityManager());
  
          let visibleDbIds = [];
          if (!implVisibilityMgr) {
            console.warn("No se encontró un visibilityManager. Se considerarán todos los elementos como visibles.");
            ...
            visibleDbIds = allDbIds.filter(dbId => {
              try {
                ...
                console.warn("Error al obtener la visibilidad del nodo:", dbId, error);
                return false;
              }
            });
          }
          ...
          this.viewer.model.getBulkProperties(visibleDbIds, ["Category"], (items) => {
